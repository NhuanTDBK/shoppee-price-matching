...
config = transformers.XLNetConfig.from_pretrained(params["model_name"])
...
def encoder(titles: Union[str]):
    tokenizer = transformers.XLNetTokenizer.from_pretrained(params["model_name"])
    ct = len(titles)
...
def create_model():
    word_model = transformers.TFXLNetModel.from_pretrained(params["model_name"], config=config)
