...
VALID_IMAGE_SIZE = (384,384)

saved_path = params["saved_path"]
model_id = "multiscale" + "_".join([params["model_name"], str(params["image_size"]), str(params["batch_size"]), str(params["optim"]),
...
def main():
    ...
    for fold_idx, (train_files, valid_files) in enumerate(cv.split(files, np.arange(n_folds))):
        ...
        ds_train = get_training_dataset_multi_scale(files[train_files], params["batch_size"], image_size=IMAGE_SIZE)
